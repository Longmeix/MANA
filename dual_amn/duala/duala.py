...
class overAll(nn.Module):
    def __init__(self, node_size, node_hidden,
        ...
        self.e_encoder = Dual(node_hidden, depth=depth, device=device)
        self.r_encoder = Dual(node_hidden, depth=depth, device=device)
        self.device = device
...
class Dual(nn.Module):
    ...
    def forward(self, g: dgl.graph, g_r: dgl.heterograph, features, rel_emb):
        ...
        for l in range(self.depth):
            ...
            neighs = neighs - 2 * torch.sum(neighs * rels_sum, 1, keepdim=True) * rels_sum  # eq.(2)
            att1 = torch.squeeze(torch.matmul(rels_sum, attention_kernel), dim=-1)

            att = torch.sparse.FloatTensor(torch.vstack(g.edges()), att1, (g.num_nodes(), g.num_nodes()))  # eq.(3)
