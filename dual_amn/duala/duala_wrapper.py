...
class DualAMNWrapper:
    def __init__(self, *args, ent_sizes=None, rel_sizes=None, triples=None, **kwargs):
        ...
        node_hidden = 128
        # node_hidden = 43  # final output_dim=43*3*2=258
        rel_hidden = 128
        batch_size = 1024
        dropout_rate = 0.3
        # lr = 0.005
    ...
    def train1step(self, epoch):
        ...
        for i in trange(epoch):
            ...
            if (i+1) % 5 == 0:
                ...
                # self.CSLS_test()
                mrr = self.eval_metrics(eval_final=False)
                if mrr > mrr_best:
                    mrr_best = mrr
                    self.output_best = self.output
                print('MRR best: {:.4f}'.format(mrr_best))

        # self.CSLS_test()
        self.eval_metrics(eval_final=True)
    ...
    def CSLS_test(self, thread_number=16, csls=10, accurate=True):
        ...
        eval_alignment_by_sim_mat(Lvec, Rvec, [1, 5, 10], thread_number, csls=0, accurate=accurate)
        return None

    def eval_metrics(self, eval_final=False):
        if eval_final:
            vec = self.output_best
        else:
            vec = self.output
        Lvec = np.array(vec[:self.ent_sizes[0], :])
        Rvec = np.array(vec[self.ent_sizes[0]:, :])
        Lvec = Lvec / np.linalg.norm(Lvec, axis=-1, keepdims=True)
        Rvec = Rvec / np.linalg.norm(Rvec, axis=-1, keepdims=True)

        train_links = self.train_pair.copy()
        train_links[:, 1] = train_links[:, 1] - self.ent_sizes[0]
        test_links = self.dev_pair.copy()
        test_links[:, 1] = test_links[:, 1] - self.ent_sizes[0]

        if eval_final:
            for k in [1, 10, 30]:
                mrr, hit_p, suc_p = self.eval_na_hit(Lvec, Rvec, train_links, test_links, k=k)
        else:
            mrr, hit_p, suc_p = self.eval_na_hit(Lvec, Rvec, train_links, test_links, k=10)
        return mrr
    ...
    def eval_na_hit(self, embed_s, embed_t, train_link, test_link, k, mask=None, default=-1.):
        ...
        from metric_uil import get_metrics, sim_cosine
        with torch.no_grad():
    ...
    # def NA_metric_test(self):
    #     vec = self.output
    #     Lvec = np.array([vec[e1] for e1, e2 in self.dev_pair])
    #     Rvec = np.array([vec[e2] for e1, e2 in self.dev_pair])
    #     Lvec = Lvec / np.linalg.norm(Lvec, axis=-1, keepdims=True)
    #     Rvec = Rvec / np.linalg.norm(Rvec, axis=-1, keepdims=True)
    #     from dual_amn.metric_uil import get_metrics
    ...
    def get_curr_embeddings(self, device=None):
        ...
        with torch.no_grad():
            ...
            return Lvec.cpu().numpy(), Rvec.cpu().numpy()
