...
class NSLoss(nn.Module):
    ...
    def get_adjacent(idx_s: Tensor, idx_t: Tensor, adj_mat) ->Tensor:
        ...
        if adj_mat is None:
            ...
            if not isinstance(idx_s, list):
                idx_s = idx_s.tolist()
            if not isinstance(idx_t, list):
                idx_t = idx_t.tolist()
    ...
    def get_xy(self, *input):
        ...
        if neg_num > 0:
            ...
            # idx_neg = self.sample(neg_num, probs, batch_size, len(embed_t))
            idx_neg = self.sample(neg_num, None, batch_size, len(embed_t))  # 均匀
