...
def cosine_np(xs, ys, epsilon=1e-8):
    """
    Efficiently calculate the pairwise cosine similarities between two set of vectors.
    Args:
        xs: feature matrix, [N, dim]
    ...
    """
    mat = xs @ ys.T
    x_norm = np.linalg.norm(xs, ord=2, axis=1) + epsilon
    y_norm = np.linalg.norm(ys, ord=2, axis=1) + epsilon
...
def np_softmax(mat):
    exp_mat = np.exp(mat - np.max(mat, axis=0, keepdims=True))
    return exp_mat / np.sum(exp_mat, axis=0, keepdims=True)