...
parser.add_argument("--task", type=str, default='d_w_15k', help="the alignment task name", choices=['en_fr_15k', 'en_de_15k', 'd_w_15k', 'd_y_15k', 'en_fr_100k', 'en_de_100k', 'd_w_100k', 'd_y_100k'])
...
#STEP: load data
kgdata = KGData(model=args.model, task=args.task, device=device, neg_samples_size=args.neg_samples_size, fold=args.fold, train_ratio=args.train_ratio/10, val=args.val, direct=args.direct)
kgdata.data_summary()
...
def evaluate_na(train_pair, test_pair, k, phase="test", model_path=None):
    ...
    eval_hit_p(sr_embedding, tg_embedding, train_pair, test_pair, k=k)
...
try:
    ...
    for e in range(args.epoch):
        ...
        if (e+1) % 10 == 0:
            ...
            eval_hit_p(sr_embedding.detach().cpu().numpy(), tg_embedding.detach().cpu().numpy(), kgdata.mapped_train_pair, kgdata.mapped_test_pair, k=10)
...
def evaluate(test_pair, k, eval_metric, phase="test", model_path=None):
    ...
    similarity_matrix = cal_distance(Lvec, Rvec, eval_metric,
                                     csls_k=args.csls_k)  # Note that the ground truth alignment relation is on the diagonal of similarity matrix
