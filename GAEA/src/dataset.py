...
from GAEA.src.utils import sparse_mx_to_torch_sparse_tensor, normalize_adj, cal_distance
...
class KGData():
    ...
    def __init__(self, model, task, device, neg_samples_size, fold, train_ratio=0.3, val=False, direct=True, lang=[1,2], save=False):
        ...
        elif "OpenEA" or 'dataset' in self.file_dir: # need to give the id manually
            if 'FT' or 'DBLP' in self.file_dir:
                save_ea_path = self.file_dir + '/EA_gaea'
                if not os.path.exists(save_ea_path):
                    print('Transfer NA data as EA format...\n')
                    self.NA_data_format(self.file_dir, train_ratio)  # transfer to EA format for NA dataset
                self.file_dir = save_ea_path
        ...
        # else:  # for datasets of network alignment


            # # --- load triples ---#
            # self.entity1, self.rel1, self.triples1 = self.load_NA_triple(f"{self.file_dir}/0_{fold}/triples_{lang[0]}")
            # self.entity2, self.rel2, self.triples2 = self.load_NA_triple(f"{self.file_dir}/0_{fold}/triples_{lang[1]}")
            #
            # # --- load ids dict ---#
            # self.ent2id, self.id2ent, [self.kg1_ent_ids, self.kg2_ent_ids],\
            #     = self.load_dict(
            # f'{self.file_dir}/0_{self.fold}/ent_ids_', file_num = 2)
            # self.rel2id, self.id2rel, [self.kg1_rel_ids, self.kg2_rel_ids] = self.load_dict(
            # f'{self.file_dir}/0_{self.fold}/rel_ids_')
        ...
        if "dbp15k" in self.file_dir:
            ...
            # for NA datasets
            self.train_pair = self.load_alignment_pair_openea(
                f"{self.file_dir}/train_ratio_{self.train_ratio}/train_links", self.ent2id)  # entity links for training/validation
            self.test_pair = self.load_alignment_pair_openea(
                f"{self.file_dir}/train_ratio_{self.train_ratio}/test_links", self.ent2id)  # entity links for testing
    ...
    def task2filedir(self, task):
        ...
        task2filedir_dict = {
            ...
            'en_fr_15k': '../dataset/OpenEA_dataset_v2.0/EN_FR_15K_V1',
            'en_de_15k': 'data/OpenEA_dataset_v2.0/EN_DE_15K_V1',
            'd_w_15k': '../dataset/OpenEA_dataset_v2.0/D_W_15K_V1',
    ...
    def NA_data_format(self, data_dir, ratio):
        ...
        save_ea_path = data_dir + '/EA_gaea'
        save_link_path = save_ea_path + f'/train_ratio_{self.train_ratio}'
        if not os.path.exists(save_link_path):
            os.makedirs(save_link_path)

        adj_path = data_dir + '/adj_{}.pkl'
        s_nodes = set()
        start_id = 0
        for i, net in enumerate(('s', 't')):
            start_id += len(s_nodes)
            adj = read_pickle(adj_path.format(net))
            nodes = list(zip(*(adj.nonzero())))
            with open(save_ea_path + f'/rel_triples_{i + 1}', 'w', encoding='utf-8') as f:
                for node in nodes:
                    triples = f'{node[0] + start_id}\tneighbor\t{node[1] + start_id}\n'
                    f.write(triples)
                    if net == 's':
                        s_nodes.add(node[0])
                        s_nodes.add(node[1])

        # links_path = data_path + f'links_{ratio}.pkl'
        links_path = data_dir + f'/links_{ratio}.pkl'
        train_links, test_links = read_pickle(links_path)
        train_links = [(pair[0], pair[1]+start_id) for pair in train_links]
        test_links = [(pair[0], pair[1]+start_id) for pair in test_links]

        writeLink2file(train_links, save_link_path + '/train_links')
        # writeLink2file(valid_links, links_path + 'valid_links')
        writeLink2file(test_links, save_link_path + '/test_links')
        return save_ea_path
    ...
    def generate_neg_sample_batch(self, pos_sn, pos_tn, sr_embedding, tg_embedding, neg_samples_size,
                                  eval_metric='inner', csls_k=0, e=0.9):
        '''
            generate negtive samples for metrics learning
            [neg1_left, neg1_right] the type1 negative samples with changed right index
            [neg2_left, neg2_right] the type2 negative samples with changed left index
            :param pos_anc: positive pair, e.g. anchor link
        '''

        # t = self.train_pair_size
        t = len(pos_sn)
        # # broadcast ground truth
        # L = np.ones((t, neg_samples_size)) * (pos_src.reshape((t, 1)))
        # neg1_left = L.reshape((t * neg_samples_size,))
        #
        # R = np.ones((t, neg_samples_size)) * (pos_tgt.reshape((t, 1)))
        # neg2_right = R.reshape((t * neg_samples_size,))
        #
        # # generate random neg-counterparts: can also use np.random.choice
        # neg1_right = np.random.randint(low=0, high=self.kg2_ent_num, size=(t * neg_samples_size))
        ...
        L = pos_sn
        pos_s_emb = sr_embedding[pos_sn]
        sr_sim_mat = cal_distance(pos_s_emb, sr_embedding, eval_metric, csls_k)
        L = L.reshape((t, 1))
        ...
        R = pos_tn
        pos_t_emb = tg_embedding[R]
        tg_sim_mat = cal_distance(pos_t_emb, tg_embedding, eval_metric, csls_k)
        R = R.reshape((t, 1))
