...
class MappingModel(nn.Module):
    def __init__(self, embedding_dim=800, hidden_dim1=1200, hidden_dim2=1600,
        ...
        #     torch.from_numpy(source_embedding).to()
    ...
    def forward(self, source_feats, mode='map'):
    ...
    def supervised_loss(self, links, alpha=1, k=5):
        source_batch, target_batch = [list(i) for i in list(zip(*links))]
        ...
        # source_feats
        # top_k_simi = self.find_topk_simi(target_feats, self.target_embedding, k=k)
        # reward_source_target += self.compute_rst(source_after_map, top_k_simi)
        # reward_target_source += self.compute_rts(target_feats, top_k_simi)
        ...
        for i in range(source_feats.shape[0]):
            ...
            # reward_target_source += torch.sum(target_after_map[i] * embedding_of_ua)
