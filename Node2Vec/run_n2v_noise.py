...
def load_data(data_dir, adj_path, save_emb_path, ratio):
    adj_s = read_pickle(adj_path.format('s')).astype(np.float32)
    adj_t = read_pickle(adj_path.format('t')).astype(np.float32)
...
def parse_args():
  ...
  parser.add_argument('--adapt', default='False', type=str2bool)
  parser.add_argument('--meta_bsz', default=8, type=int)
  ...
  parser.add_argument('--ptb_p', default=0.15, type=float)
  parser.add_argument('--n_way', default=5, type=int)
  parser.add_argument('--k_shot', default=5, type=int)
  parser.add_argument('--mapping_lr', default=5e-4, type=float)
...
if __name__ == '__main__':
    ...
    ptb_dir = data_dir + f'attack/{args.ptb_p}/'
    adj_path = ptb_dir + 'adj_ptb_{}.pkl'
    save_emb_path = ptb_dir + 'emb_n2v_ptb1.pkl'
    ...
    train_align(*load_data(data_dir, adj_path, save_emb_path, args.ratio), args)