...
from JMAC_EA.modules.finding.evaluation import valid, test, early_stop
from JMAC_EA.modules.helper.helper import *
from JMAC_EA.modules.load.data_loader import *

from JMAC_EA.models.basic_model import BasicModel
from JMAC_EA.models.jmac_model import JMAC_MODEL
from JMAC_EA.modules.utils.util import get_neg
from collections import defaultdict as ddict
from JMAC_EA.modules.utils.metric_uil import eval_na_hit
...
class Trainer(BasicModel):
        ...
        for i in range(1, self.args.max_epoch + 1, self.args.eval_freq):
            ...
            for j in range(self.args.eval_freq): 
                ...
                if step % self.args.eval_freq == 0:
                    ...
                    mrr, hit_p, success_p = self.eval_na_metrics(align_emb, eval_final=False, in_list=self.prox_acc)

                    if mrr > mrr_best:
                        mrr_best = mrr
                        hit_best = hit_p
                        success_best = success_p
                        self.align_emb_best = align_emb
                    print('MRR_best: {:.4f}'.format(mrr_best))
                    self.logger.info(
                        '== Common best == MRR: {:.4f}, Hit: {:.4f}, Success:{:.4f}'.format(mrr_best, hit_best, success_best))
    ...
    def eval_na_hit(self, embed_s, embed_t, train_link, test_link, k, mask=None, default=-1.):
        """
        Evaluation precision@k and hit-precision@k in the training set and testing set.
        """
        from JMAC_EA.modules.utils.metric_uil import get_metrics, sim_cosine
        with torch.no_grad():
            similarity_mat = sim_cosine(embed_s, embed_t)
            if mask is not None:
                similarity_mat *= mask
            mrr, hit_p, suc_p = get_metrics(similarity_mat, k, train_link)
            self.logger.info('Train MRR {:.4f} | Hit@{} {:.4f} | Success@{} {:.4f}'.format(
                mrr, k, hit_p, k, suc_p
            ))
            ...
            self.logger.info('Test MRR {:.4f} | Hit@{} {:.4f} | Success@{} {:.4f}'.format(
                mrr, k, hit_p, k, suc_p
            ))
            return mrr, hit_p, suc_p

    def eval_na_metrics(self, embedding, eval_final=False, in_list=[]):
        embeds1 = np.array([embedding[e] for e in self.kgs.kg1.entities_list])
        embeds2 = np.array([embedding[e] for e in self.kgs.kg2.entities_list])
        train_and_valid = np.array(self.kgs.train_links + self.kgs.valid_links)
        train_and_valid[:, 1] = train_and_valid[:, 1] - self.kgs.kg1.entities_num
        test_links = np.array(self.kgs.test_links)
        # test_links[:, 1] = test_links[:, 1] - self.kgs.kg2.entities_num
        test_links[:, 1] = test_links[:, 1] - self.kgs.kg1.entities_num

        if eval_final:
            for k in [1, 10, 30]:
                mrr, hit_p, suc_p = self.eval_na_hit(embeds1, embeds2, train_and_valid, test_links, k)
        else:
            mrr, hit_p, suc_p = self.eval_na_hit(embeds1, embeds2, train_and_valid, test_links, k=10)
        in_list.append(mrr)
        return mrr, hit_p, suc_p
    ...
    def run(self):
        ...
        # self.test(align_emb)
        # mrr_best = self.report_hit(self.align_emb_best)
        mrr_best, _, _ = self.eval_na_metrics(self.align_emb_best, eval_final=True)
        return mrr_best

    # def report_hit(self, embedding):
    #     embeds1 = np.array([embedding[e] for e in self.kgs.kg1.entities_list])
    #     embeds2 = np.array([embedding[e] for e in self.kgs.kg2.entities_list])
    #     train_and_valid = self.kgs.train_links + self.kgs.valid_links
    #     mrr_best = .0
    #     with torch.no_grad():
    #         # for k in [1] + [10 * i for i in range(1, 6)]:
    #         for k in [1, 10, 30]:
    #             mrr_best, _, _ = self.eval_na_hit(embeds1, embeds2, train_and_valid, self.kgs.test_links, k)
    #     return mrr_best
    ...
    def load_ent_name_emb(self):
        ...
        if self.args.no_attr_info:
            ...
            return torch.FloatTensor(np.zeros((self.num_ent, self.emb_dim)) + 0.03).to(self.device)
    ...
    def _get_desc_input(self): 
        ...
        if self.args.no_name_info:
            ...
            return None, None, np.zeros((self.num_ent, self.emb_dim))
