...
# parser.add_argument('--training_data', type=str, default='../../../dataset/OpenEA_dataset_v2.0/D_W_15K_V1/')
...
parser.add_argument('--no_name_info', type=bool, default=True, help='whether to exclude name information')
...
if __name__ == '__main__':
    ...
    if args.no_name_info:
        extend_name = 'no_name'
    args.training_data = f'../dataset/{args.dataset}/EA_data/'
    args.dataset_division = f'train_ratio_{args.train_ratio}/'
    # args.name = args.training_data.split('/')[-2] + extend_name
    # logger = Logger(args.name, logging.INFO, True, True)
    from utils.log import create_logger
    from time import strftime, localtime
    args.logname = strftime("logs/JMAC_{}_tr{}_%m-%d_%H:%M:%S.txt".format(
        args.dataset, args.train_ratio
    ), localtime())
    logger = create_logger(
        __name__, silent=False,
        to_disk=True, log_file=args.logname)
    ...
    trainer.init()
    mrr_best = trainer.run()

    from utils.log import rename_log
    rename_log(args.logname, '/mrr{:.2f}_'.format(mrr_best * 100).join(
        args.logname.split('/')
    ))
