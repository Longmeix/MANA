...
def generate_ent_id(kg1_triples, kg1_elements, kg2_triples, kg2_elements, ordered=True):
    ids1, ids2 = dict(), dict()
    kg1_ent_deg_dict, kg2_ent_deg_dict = dict(), dict
    if ordered:
        kg1_ordered_elements, kg1_ent_deg_dict = sort_elements(kg1_triples, kg1_elements)
        kg2_ordered_elements, kg2_ent_deg_dist = sort_elements(kg2_triples, kg2_elements)
        n1 = len(kg1_ordered_elements)
        n2 = len(kg2_ordered_elements)
        for i in range(n1):
            # ids1[kg1_ordered_elements[i]] = int(kg1_ordered_elements[i])
            ids1[kg1_ordered_elements[i]] = i
        for i in range(n2):
            # ids2[kg2_ordered_elements[i]] = n1 + int(kg2_ordered_elements[i])
            ids2[kg2_ordered_elements[i]] = n1 + i
    else:
        index = 0
        for ele in kg1_elements:
            if ele not in ids1:
    ...
    assert len(ids1) == len(set(kg1_elements))
    assert len(ids2) == len(set(kg2_elements))
    return ids1, ids2, kg1_ent_deg_dict, kg2_ent_deg_dict
...
def read_kge_dataset(file_path):
    print("read relation triples:", file_path)
    if file_path is None:
        return set(), set(), set()
    triples = set()
...
def save_special_embeddings(folder, namee, namer, e_embeds, r_embeds):
    if not os.path.exists(folder):
        os.makedirs(folder)
    if e_embeds is not None:
        np.save(folder + namee + '.npy', e_embeds)
    if r_embeds is not None:
        np.save(folder + namer + '.npy', r_embeds)
